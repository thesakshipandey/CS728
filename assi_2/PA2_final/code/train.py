...
def _sigmoid_sat_dist(v: torch.Tensor) -> torch.Tensor:
    # v in [0,1]
    return torch.minimum(v, 1.0 - v).clamp(min=0.0)


def _tanh_sat_dist(v: torch.Tensor) -> torch.Tensor:
    # v in [-1,1]
    return (1.0 - v.abs()).clamp(min=0.0)
...
def omega_regularizer_and_gradW_hh(model, loss: torch.Tensor, h: torch.Tensor, bound: float):
    ...
    dH_tuple = torch.autograd.grad(loss, h, create_graph=True, retain_graph=True, allow_unused=True)
    dH = dH_tuple[0] if dH_tuple[0] is not None else torch.zeros_like(h)
    ...
    gW_hh_tuple = torch.autograd.grad(omega, model.W_hh, retain_graph=True, create_graph=False, allow_unused=True)
    gW_hh = gW_hh_tuple[0] if gW_hh_tuple[0] is not None else torch.zeros_like(model.W_hh)
...
def grad_time_profile(task, model, x: torch.Tensor, y_onehot: torch.Tensor, collect_extras: bool = False):
    ...
    dH_or_none = torch.autograd.grad(loss, h, retain_graph=True, allow_unused=True)[0]
    if dH_or_none is None:
        dH = torch.zeros_like(h)
    else:
        dH = dH_or_none

    g_t = (dH * dH).sum(dim=2).mean(dim=1)
    ...
    z_sat_t = None
    r_sat_t = None
    if collect_extras and extras is not None and isinstance(extras, dict):
        z_gates = torch.sigmoid(extras["z"])
        r_gates = torch.sigmoid(extras["r"])
        z_sat_t = _sigmoid_sat_dist(z_gates).mean(dim=(1, 2))
        r_sat_t = _sigmoid_sat_dist(r_gates).mean(dim=(1, 2))
...
def parse_args():
    ...
    p.add_argument("--alpha", type=float, default=0.0, help="Omega regularizer strength (applied to W_hh only).")
    ...
    p.add_argument("--maxiters", type=int, default=int(50000), help="Training iterations (batches).")
    p.add_argument("--saveFreq", type=float, default=5.0, help="Save every N minutes.")
    p.add_argument("--name", type=str, default="test_torch")
    p.add_argument("--resume", type=str, default="", help="Resume from a saved .npz checkpoint.")
...
def _restore_model_from_payload(model, payload: dict[str, np.ndarray], device: str) -> None:
    state = model.state_dict()
    for key, value in state.items():
        source = None
        if key in payload:
            source = payload[key]
        elif f"state__{key}" in payload:
            source = payload[f"state__{key}"]
        if source is not None:
            state[key] = torch.tensor(source, dtype=value.dtype, device=device)
    model.load_state_dict(state, strict=False)


def _copy_1d(dst: np.ndarray, src: np.ndarray | None) -> None:
    if src is None:
        return
...
def _copy_2d(dst: np.ndarray, src: np.ndarray | None) -> None:
    if src is None:
        return
    nr = min(dst.shape[0], src.shape[0])
    nc = min(dst.shape[1], src.shape[1])
    if nr > 0 and nc > 0:
        dst[:nr, :nc] = src[:nr, :nc]


def _advance_training_stream(args, rng: np.random.RandomState, task, start_iter: int) -> None:
    for _ in range(start_iter):
        if args.max_length > args.min_length:
            length = args.min_length + rng.randint(args.max_length - args.min_length)
        else:
            length = args.min_length
        task.generate(args.bs, length)
...
def main():
    ...
    best_score = 100.0
    if args.resume:
        payload = _load_payload(args.resume)
        _restore_model_from_payload(model, payload, device)

        _copy_1d(store_train, payload.get("train_nll"))
        _copy_1d(store_valid, payload.get("valid_error"))
        _copy_1d(store_norm, payload.get("gradient_norm"))
        _copy_1d(store_rho, payload.get("rho_Whh"))
        _copy_1d(store_reg, payload.get("Omega"))
        _copy_1d(store_steps, payload.get("steps_in_past"))
        _copy_2d(store_grad_time, payload.get("grad_time"))
        _copy_2d(store_act_time, payload.get("act_time"))
        _copy_2d(store_sat_time, payload.get("sat_time"))
        _copy_2d(store_gate_z_sat_time, payload.get("gate_z_sat_time"))
        _copy_2d(store_gate_r_sat_time, payload.get("gate_r_sat_time"))
        _copy_1d(store_diag_loss, payload.get("diag_loss"))
        _copy_1d(store_diag_err, payload.get("diag_err"))

        train_mask = np.isfinite(store_train) & (store_train >= 0.0)
        valid_mask = np.isfinite(store_valid) & (store_valid >= 0.0)
        start_iter = int(train_mask.sum())
        if valid_mask.any():
            best_score = float(store_valid[valid_mask].min())
        _advance_training_stream(args, rng, task, start_iter)
        print(f"Resuming from {args.resume} at iter {start_iter}.")
        ...
        if n % args.checkFreq == 0 and n > 0:
            ...
            if pos < store_space:
                ...
                if args.collectDiags:
                    ...
                    print(
                        f"  [diag] log10 g_t hist bins={args.diagBins} range=({g_edges[0]:.2f},{g_edges[-1]:.2f}) "
