...
def initialization_scheme(hidden_shape, device):
...
def spectral_radius(mat: np.ndarray) -> float:
    ...
    # mat is (H,H)
    return  torch.max(torch.abs(torch.linalg.eigvals(torch.tensor(mat)))).item()
...
class VanillaRNN(nn.Module):
    ...
    def __init__(
        ...
        self.dtype = dtype
        if init == "sigmoid":
            # this has partially implemented Pascanu et al.'s suggestions to dissuade exploding grads
    ...
    def forward(self, u: torch.Tensor):
        ...
        # this ought to be implemented using a for loop right ?
        curr_hidden_state = initialization_scheme((B, self.nhid), device=self.device)
        # output_record = []
        activation_record = []
        for timestep in range(T):
            curr_hidden_state = self.act(curr_hidden_state @ self.W_hh + u[timestep, :, :] @ self.W_uh + self.b_hh)
            # output = curr_hidden_state @ self.W_hy + self.b_hy
            # output_record.append(output)
            activation_record.append(curr_hidden_state)
        
        tensorified_activations = torch.stack(activation_record, dim=0)
        tensorified_activations.to(device=self.device)
        
        # flattened = tensorified_activations.reshape((T*B, self.nhid))
        # print(output_record[-1].shape)
        # time.sleep(5)

        if self.classif_type == 'lastSoftmax' or self.classif_type == 'lastLinear':
            tensorified_output = tensorified_activations @ self.W_hy + self.b_hy
            return tensorified_output[-1,  :, :], tensorified_activations
        
        tensorfied_outputs = tensorified_activations @ self.W_hy + self.b_hy
        tensorfied_outputs.to(device=self.device)
        # print(tensorfied_outputs.shape)
        # time.sleep(5)
        return tensorfied_outputs.reshape((T*B, self.nout)), tensorified_activations 
