...
# DO THIS
def grad_time_profile(task, model:torch.nn.Module, x: torch.Tensor, y_onehot: torch.Tensor, collect_extras: bool = False):
    """
    ...
    loss, err, y, hidden_state_activations , extras = compute_loss_and_error(task, model,  x, y_onehot, return_extras=collect_extras)
    
    # try:
    #     _, hidden_state_activations = model(x, return_extras=collect_extras)
    # except TypeError:
    #     _, hidden_state_activations = model(x)
    
    
    hidden_state_activations.retain_grad()
    loss.backward()
    
    B = hidden_state_activations.shape[1]
    hidden_state_grad_norms = torch.linalg.norm(hidden_state_activations.grad, dim=2)
    g_t = torch.sum(hidden_state_grad_norms, dim=1)/B
    activation_derivatives = model.act_deriv_from_h(hidden_state_activations)
    a_t = activation_derivatives.mean(dim=(1, 2))
    sat_t = _hidden_sat_time(model, hidden_state_activations)
    # what is err?
    z_sat_t = None 
    r_sat_t = None
    if model.model_name == 'gru':
        # Shubham bhai idhar update karde GRU ka code 
        pass
...
def global_grad_norm(params):
    ...
    # max_norm = 1e9
    # total_norm = clip_grad_norm_(params, max_norm, norm_type=2)
    # return total_norm
    normie = 0.0
    for param in params:
        if param.grad is not None:
            normie += param.grad.detach().pow(2).sum()
    return torch.sqrt(normie)
